...
function renderLangSwitcher() {
    return `
            <div class="absolute top-2 right-2 flex gap-2 z-50">
                <button onclick="setLanguage('it')" class="w-8 h-8 text-xl rounded-full border-2 border-white shadow hover:scale-110 transition ${gameState.language === 'it' ? 'ring-2 ring-yellow-400' : 'opacity-70'}">🇮🇹</button>
                <button onclick="setLanguage('en')" class="w-8 h-8 text-xl rounded-full border-2 border-white shadow hover:scale-110 transition ${gameState.language === 'en' ? 'ring-2 ring-yellow-400' : 'opacity-70'}">🇬🇧</button>
                <button onclick="setLanguage('fr')" class="w-8 h-8 text-xl rounded-full border-2 border-white shadow hover:scale-110 transition ${gameState.language === 'fr' ? 'ring-2 ring-yellow-400' : 'opacity-70'}">🇫🇷</button>
            </div>
        `;
}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-grow overflow-hidden">
                ...
                <div class="lg:col-span-3 flex flex-col gap-2 overflow-y-auto">
                    ...
                    <div class="vietri-card p-4 bg-white border-yellow-400 text-center relative flex-grow">
                        <div class="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-yellow-400 text-white text-xs px-2 py-1 rounded">${t('commission_label')}</div>
                        ...
                        <div class="text-sm text-gray-700 bg-gray-50 p-2 rounded mb-2 text-left space-y-1">
                            <div id="obj-req">${t('req_label')} -</div>
                        </div>
                        <div class="text-xs text-green-600 font-bold" id="obj-reward">${t('reward_label')} -</div>
                    </div>
                </div>
...
function modSlot(type, index, item, amount) {
    if (type === 'inner') {
        const slot = gameState.innerSlots[index]; // Fixed reference
        if (amount > 0) {
            if (gameState.clay >= amount) {
                gameState.clay -= amount;
                slot.clay += amount;
            ...
            if (slot.clay >= -amount) {
                gameState.clay -= amount; // amount is negative, so this adds
                slot.clay += amount;
            }
        }
    } else {
        const slot = gameState.outerSlots[index]; // Fixed reference
        if (amount > 0) {
            // Add
            if (gameState.pigments[item] > 0) {
                // Check Max Types Constraint (Max 2 types)
                const currentTypes = Object.values(slot).filter(v => v > 0).length;
                if (slot[item] > 0 || currentTypes < 2) {
                    gameState.pigments[item]--;
                    slot[item]++;
